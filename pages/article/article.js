...
/*百度网盘*/
    baiduDiskClick: function (e) {
        if (!this.data.post.baidu_disk || !this.data.post.baidu_disk.url || !this.data.post.baidu_disk.name) {
            return;
        }

        if (this.data.post.baidu_disk.jili == 1 && this.data.show_baidu_jili) {
            this.adJiliClick();
        } else {
            let url = `plugin-private://wx8c873f830774d652/pages/outsite-chain/share-code/index?url=${this.data.post.baidu_disk.url}&origin=${this.data.post.baidu_disk.name}`;
            if (this.data.post.baidu_disk.code) {
                url += `&sharecode=${this.data.post.baidu_disk.code}`;
            }
            if (this.data.post.baidu_disk.home) {
                url += `&home=${this.data.post.baidu_disk.home}`;
            }

            wx.setClipboardData({
                data: this.data.post.baidu_disk.code,
                success(res) {
                    Util.toast('提取码已复制');
    ...
    goexceptional: function () {
        wx.previewImage({
          urls: ['https://bbsmm.mm-app.com/faa87e4f4007b1f64aee83a1a9f723d.png']
        })
      },
